...
must_haves:
  truths:
    ...
    - "Teacher proposes remove actions for unhelpful skills as part of analysis"
...
`__init__(self, skill_library_path, output_dir, max_iterations=20, patience=5, min_delta=0.01, max_concurrent=10, max_steps=50, top_k_skills=3, wandb_project="skillrl-evolution")`:
- Store all config
...
3. For iteration in range(max_iterations):
   ...
   f. If NOT last iteration:
      ...
      - Apply proposals to skill library (see _apply_proposals below) — this includes teacher "remove" proposals for pruning
...
**Skill pruning strategy:** Pruning is handled entirely by teacher "remove" proposals. The teacher prompt (from Plan 03-01) already instructs the teacher to propose "remove" actions for unhelpful skills. No automatic usage-based pruning — the teacher has full context to decide which skills to remove. This avoids complex usage tracking across the evaluation pipeline.
