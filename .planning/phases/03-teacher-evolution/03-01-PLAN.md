...
1. `FAILURE_ANALYSIS_PROMPT` -- System prompt for analyzing failed trajectories. The prompt must:
   ...
   - Include few-shot examples showing GOOD skills vs BAD skills. Concrete examples:
     - GOOD: "Always verify you are holding an object before attempting to place it somewhere"
     - GOOD: "When searching for an object, check likely containers systematically rather than randomly exploring"
     - GOOD: "After picking up an object, confirm it is in inventory before proceeding to the next step"
     - BAD: "Take tomato 1 from countertop 3" (mentions specific object instance and location)
     - BAD: "Check cabinet 3 for the mug" (mentions specific receptacle and object)
     - BAD: "Go to fridge 1 to find lettuce" (mentions specific location and object)
