...
<task type="auto">
  ...
  <action>
    1. **Update `src/agent/loop.py`** — Add `retrieved_skills` parameter to `run_task()`:
       - Add parameter: `retrieved_skills: list | None = None`
       - Import `build_prompt_with_skills` from `src.agent.prompts`
       - Replace the hardcoded `AUTONOMOUS_AGENT_PROMPT` in the messages list with `build_prompt_with_skills(retrieved_skills)`. This way, when skills are passed, they get injected; when None/empty, the base prompt is used.
